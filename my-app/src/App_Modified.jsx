...
function App() {
  ...
  return (
    <React.Fragment>
      ...
      {<div className="m-2">Left hand average finger joint angle: {leftHandOpenness}</div>}
      <Button className="m-2" onClick={() => setAudioStatus(!audioStatus)}> audioStatus: {audioStatus ? 'On' : 'Off'} </Button>
      <Tones rightHandPos={rightHandPos} rightHandOpenness={rightHandOpenness} leftHandPos={leftHandPos} leftHandOpenness={leftHandOpenness} audioStatus={audioStatus} />
    </React.Fragment>
  );
}
